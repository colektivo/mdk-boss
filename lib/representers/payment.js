...
var Payment = {
  compare: function(hours){
    return models.sequelize.query(
      "select *, CAST( :hours * (monthly_income / 160) AS float) as payment from salaries ORDER BY payment DESC"
      , {
  ...
  compute: function(card_id, maxDevices, timestamp) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      "    (interval '1 hour') as cycle_time_interval, " +
      "    (interval '5 minutes') as time_window, " +
      "    (interval '6 hours') as time_window, " +
      "    (interval '10 minutes') as minimal_time_between_devices, " +
      "    (interval '5 hours') as max_time_in_the_space, " +
      "    row_number() OVER (PARTITION by card_id ORDER BY created_at DESC) as myrow, " +
      ...
      "SELECT *, " +
      "  lag(created_at,1) OVER (PARTITION by card_id) - created_at as time_spent, " +
      "  ROUND(EXTRACT(epoch FROM lag(created_at,1) OVER (PARTITION by card_id) - created_at)) as time_spent_in_seconds " +
      "FROM ( SELECT " +
      "  id, " +
      "  card_id, " +
      "  myrow, " +
      ...
      return models.sequelize.query(magicQuery2,
        ...
        if (tracks.length > 0) {
          ...
          var startDate = moment(tracks[tracks.length - 1].created_at).utc().toDate();
          ...
          var timeReport = tracks.slice(1).reverse().map(function(track, index, tracks){
            return { position: track.position, timeSpent: track.time_spent, timeSpentInSeconds: track.time_spent_in_seconds, happened: track.created_at };
          });

          timeUsed = { happened: startDate, workingTime: workingTime, timeReport: timeReport, isValid: isValid, isComplete: isComplete };
        ...
        if (timeUsed.isValid) {
          Payment.compare(timeUsed.workingTime.decimalTime).then( function (result) {
            timeUsed.salaries = result.map(function(salary, index){
              return {
                text: salary.text
            ...
            console.log(JSON.stringify(timeUsed));
            Stats.compute().then(function(stats){
              timeUsed.stats = stats;
              resolve(timeUsed);
            });
          });
        } else {
